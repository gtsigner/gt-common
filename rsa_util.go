...
func RsaPubEncrypt(publicKey *rsa.PublicKey, origData []byte) ([]byte, error) {
...
func (rsa *Rsa) EncryptBytes(data []byte) ([]byte, error) {
    res, err := RsaEncrypt(rsa.publicKey, data)
    if err != nil {
        return nil, err
    }
    return res, nil
